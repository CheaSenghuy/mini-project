...
queryData(){
    ...
    -C $CHANNEL_NAME --name ${CC_NAME} \
    -c  '{"Args":["ReadAsset1","asset1"]}'
    # -c  '{"Args":["GetAllAssets"]}'
}

updateChainCode(){
    peer chaincode upgrade -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile ${ORDERER_CA} \
    --name ${CC_NAME} -C $CHANNEL_NAME -v ${CC_VERSION}  \
    -c '{"Args":["ReadAsset1"]}' -p "and ('Org1MSP.admin','Org2MSP.admin')"
